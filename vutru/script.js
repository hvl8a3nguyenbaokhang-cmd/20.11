...
function createAudioForCurrent() {
  ...
  // debug
  console.log('createAudioForCurrent ->', src);
}

function playGalaxyAudio() {
  if (!galaxyAudio) {
    // bảo đảm tạo audio trong ngữ cảnh user gesture nếu gọi trực tiếp
    preloadGalaxyAudio(Math.floor(Math.random() * audioSources.length));
  }
  if (galaxyAudio) {
    galaxyAudio.play().then(() => {
      console.log('galaxyAudio.play() success', galaxyAudio.src);
    }).catch(err => {
      console.warn("Audio play blocked or delayed:", err);
      // thêm log trạng thái để debug
      try { console.log('galaxyAudio.readyState', galaxyAudio.readyState, 'paused', galaxyAudio.paused); } catch(e) {}
    });
  } else {
    console.warn('playGalaxyAudio: galaxyAudio is null');
  }
}
// ...existing code...
function onCanvasClick(event) {
  ...
  // chấp nhận cả touch và click; chuẩn hóa event clientX/Y
  let clientX, clientY;
  ...
  if (intersects.length > 0) {
    ...
    // TẠO/PRELOAD audio TRONG NGỮ CẢNH USER GESTURE (quan trọng cho iOS/Android)
    if (!galaxyAudio) {
      preloadGalaxyAudio(Math.floor(Math.random() * audioSources.length));
    }

    // Chạy play trực tiếp ở đây
    playGalaxyAudio();
...
// Thêm 1 lần lắng nghe cho click và touchstart (loại bỏ lắng nghe trùng)
// Nếu đã có dòng listener khác: xóa dòng trùng để tránh double-bind.
renderer.domElement.removeEventListener('click', onCanvasClick);
renderer.domElement.removeEventListener('touchstart', onCanvasClick);
renderer.domElement.addEventListener('click', onCanvasClick, { passive: true });
renderer.domElement.addEventListener('touchstart', onCanvasClick, { passive: true });
// ...existing code...
// ...existing code...
